...
<xsl:stylesheet
	...
	<xsl:template match="/">
		<html lang="hr">
			<head>
				<title>XML mapa web-stranice</title>
				<style>
			...
			<body>
				<div id="sitemap">
					...
					<div id="sitemap__content">
						<p class="text">Broj URL-ova u ovoj XML mapi web-stranice: <xsl:value-of select="count( sitemap:urlset/sitemap:url )" />.</p>
						<table id="sitemap__table">
							<thead>
								<tr>
									<th class="loc">URL</th>
									<xsl:if test="$has-lastmod">
										<th class="lastmod">Zadnja izmjena</th>
									</xsl:if>
									<xsl:if test="$has-changefreq">
										<th class="changefreq">Promijeni frekvenciju</th>
									</xsl:if>
									<xsl:if test="$has-priority">
										<th class="priority">Prioritet</th>
									</xsl:if>
								</tr>
							</thead>
							<tbody>
